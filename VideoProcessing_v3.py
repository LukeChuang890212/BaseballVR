...
def detect(frame,kernel,avg,avg_float,lightness,lowerbH,lowerbS,lowerbV,upperbH,upperbS,upperbV,lowerth,upperth,right_x_grid,left_x_grid,upper_y_grid,lower_y_grid,yellow_grid_len,grid_len,gridok,ball_in,ball_list,record):
    ...
    if gridok:
        ...
        # print("I count {} rects in this image".format(len(cnts)))
    ...
    bias = grid_len*3-yellow_grid_len*3
    cos_theta = (yellow_grid_len*3-bias)/(yellow_grid_len*3)
    ...
    if not ball_in:
        ...
        if not trackable: 
            ...
            if time.time()-start_not_track_time < 8:
                ...
                pd.DataFrame({'index':[len(record)],'x':[ball_pos[0]-left_x_grid],'y':[ball_pos[1]-upper_y_grid]}).to_csv(record_output_file+name+".csv",index=False,header=False,mode='a')
...
# ==========================
if read_mode == 0:
    cap = cv2.VideoCapture(record_output_file+'Luke-test3-l.avi')
elif read_mode == 1:
    cap = Cap()

date = "2021/10/31"
name = "莊明儒"
data = pd.DataFrame(columns=["index",'x','y'])
data.to_csv(record_output_file+name+".csv",index=False)

winName = "Colors of Rainbow"
def nothing(x):
    pass
...
cv2.createTrackbar("LowerbH", winName, 18, 255, nothing) 
...
print("lower_y_grid:",lower_y_grid)
print("upper_y_grid:",upper_y_grid)
print("left_x_grid:",left_x_grid)
print("gridok:",gridok)
print("lightness:",lightness)
